...
const navItems = [
  ...
  { label: "Leave Tracker", href: "/dashboard/leave/leave-policies", icon: CalendarDays },
  { label: "Attendance", href: "/dashboard/attendance/today", icon: Fingerprint },
  { label: "Payroll", href: "/dashboard/payroll/run", icon: Wallet },
  { label: "Shift Management", href: "/dashboard/shift-management/add-shift", icon: Timer },
  { label: "HR Letter", href: "/dashboard/hr_letter", icon: FileText },
  { label: "Settings", href: "/dashboard/settings/working-hours", icon: SlidersHorizontal },
  { label: "Company Profile", href: "/dashboard/company_profile", icon: Building2 },
...
export default function Sidebar() {
  ...
  const isSettingsModule = pathname?.startsWith("/dashboard/settings");
  const isShiftModule = pathname?.startsWith("/dashboard/shift-management");
  const isAttendanceModule =
    pathname?.startsWith("/dashboard/attendance") ||
    pathname?.startsWith("/dashboard/attendence");
  const isPayrollModule = pathname?.startsWith("/dashboard/payroll");
  const isAnnouncementsModule = pathname?.startsWith("/dashboard/announcements");
  const isApprovalConfigModule = pathname?.startsWith("/dashboard/approval_config");
  const isHrLetterModule = pathname?.startsWith("/dashboard/hr_letter");
      ...
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3">
        {navItems.map((item) => {
          ...
          const isActive =
            ...
            : item.href === "/dashboard/employees" ? isEmployeeModule
            : item.href === "/dashboard/leave/leave-policies" ? isLeaveModule
            : item.href === "/dashboard/attendance/today" ? isAttendanceModule
            : item.href === "/dashboard/payroll/run" ? isPayrollModule
            : item.href === "/dashboard/announcements" ? isAnnouncementsModule
            : item.href === "/dashboard/approval_config" ? isApprovalConfigModule
            : item.href === "/dashboard/hr_letter" ? isHrLetterModule
            : item.href === "/dashboard/Onboarding" ? isOnboardingModule
            : item.href === "/dashboard/locations" ? isLocationsModule

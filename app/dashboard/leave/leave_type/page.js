...
/**
 * Format FastAPI errors — handles string + array (422) + fallback.
 */
function getErrorMessage(err) {
  const detail = err?.response?.data?.detail;
...
export default function LeaveTypesPage() {
  ...
  const fetchLeaveTypes = useCallback(async () => {
    ...
    try {
      ...
      // Backend returns: { total_count, page, page_size, leave_types: [...] }
      const items = Array.isArray(body.leave_types) ? body.leave_types : [];
      const totalCount = Number(body.total_count) || items.length;
  ...
  async function handleSubmit(e) {
    ...
    const body = {
      ...
      leave_type_code: form.leave_type_code.trim().toUpperCase(),
      leave_type_description: form.leave_type_description.trim(),
      is_paid: form.is_paid,
      is_active: form.is_active,
    };
  ...
  return (
    <div className="min-h-screen bg-[#f5f6f8] p-6">
      ...
      {/* ══════════ FEEDBACK BANNERS ══════════ */}
      {error && !showForm && (
        <div className="mb-4 flex items-start gap-3 rounded-md bg-[#fef2f2] px-4 py-3 text-sm text-[#b91c1c]">
          <span className="flex-1">{error}</span>
          <button
            onClick={() => setError("")}
            className="text-[#b91c1c] opacity-60 hover:opacity-100"
            aria-label="Dismiss"
          >
            ✕
          </button>
      ...
      {success && (
        <div className="mb-4 flex items-start gap-3 rounded-md bg-green-50 px-4 py-3 text-sm text-green-700">
          <span className="flex-1">✓ {success}</span>
          <button
            onClick={() => setSuccess("")}
            className="text-green-700 opacity-60 hover:opacity-100"
            aria-label="Dismiss"
          >
            ✕
          </button>
      ...
      <form onSubmit={handleSearchSubmit} className="mb-4 flex flex-wrap gap-3">
        <div className="relative w-full max-w-xs">
          <input
            ...
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            className="w-full rounded-md border border-[#d1d5db] bg-white px-3 py-2 pr-9 text-sm outline-none focus:border-[#E42527]"
          />
          {searchInput && (
            <button
              type="button"
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-sm text-[#6b7280]">
              {search ? "No leave types match your search" : "No leave types yet"}
            </p>
            {!search && (
              <button
                onClick={openCreateForm}
                ...
                <tbody className="divide-y divide-[#f3f4f6]">
                  {leaveTypes.map((lt) => (
                    <tr key={lt.leave_type_id} className="hover:bg-[#fafafa]">
                      ...
                      <td className="px-5 py-3.5">
                        <span className="rounded bg-[#f3f4f6] px-1.5 py-0.5 font-mono text-xs">
                          {lt.leave_type_code}
                        </span>
                      </td>
                      <td
                        className="max-w-xs truncate px-5 py-3.5 text-[#4b5563]"
                        title={lt.leave_type_description}
                      >
                        {lt.leave_type_description || "—"}
                      </td>
                      <td className="px-5 py-3.5">
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-[#6b7280]">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              disabled={!canPrev}
              className="rounded-md border border-[#d1d5db] bg-white px-3 py-1.5 text-sm hover:bg-[#f9fafb] disabled:cursor-not-allowed disabled:opacity-50"
            >
              Previous
            </button>
            <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-5">
              <div>
                ...
                <input
                  ...
                  value={form.leave_type_name}
                  onChange={(e) =>
                    setForm({ ...form, leave_type_name: e.target.value })
                  }
                  className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm outline-none focus:border-[#E42527]"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      leave_type_code: e.target.value.toUpperCase(),
                    })
                  }
                  className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 font-mono text-sm uppercase outline-none focus:border-[#E42527]"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  value={form.leave_type_description}
                  onChange={(e) =>
                    setForm({ ...form, leave_type_description: e.target.value })
                  }
                  className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm outline-none focus:border-[#E42527]"
                  rows={3}
                />
              ...
              <div className="flex flex-col gap-3 rounded-lg bg-slate-50 p-3">
                <label className="flex cursor-pointer items-center gap-2 text-sm text-[#374151]">
                  <input
                    ...
                    className="h-4 w-4 rounded border-[#d1d5db] text-[#E42527]"
                  />
                  <span>
              ...
              {error && (
                <div className="rounded-md bg-[#fef2f2] px-3 py-2 text-sm text-[#b91c1c]">
                  {error}
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  onClick={closeForm}
                  disabled={saving}
                  className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm hover:bg-[#f9fafb] disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="rounded-md bg-[#E42527] px-4 py-2 text-sm text-white hover:bg-[#c91f21] disabled:opacity-60"
                >
                  {saving ? "Saving..." : editingId ? "Update" : "Create"}
                </button>
              </div>
      ...
      {selectedLeaveType && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setSelectedLeaveType(null);
          }}
        >
          <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div className="min-w-0">
                ...
                <h2 className="mt-1 truncate text-lg font-bold text-slate-800">
                  {selectedLeaveType.leave_type_name || "Unnamed leave type"}
                </h2>
              </div>
              <button
                type="button"

...
//               <defs>
//                 <linearGradient id={`sk-${accent}`} x1="0" y1="0" x2="0" y2="1">
//                   <stop offset="0%" stopColor={a.label.includes("red") ? "#E42527" : a.label.includes("green") ? "#10b981" : "#6366f1"} stopOpacity={0.4} />
//                   <stop offset="100%" stopColor={a.label.includes("red") ? "#E42527" : a.label.includes("green") ? "#10b981" : "#6366f1"} stopOpacity={0} />
//                 </linearGradient>
//               </defs>
//               <Area
//                 type="monotone"
//                 dataKey="v"
//                 stroke={a.label.includes("red") ? "#E42527" : a.label.includes("green") ? "#10b981" : "#6366f1"}
//                 strokeWidth={1.8}
//                 fill={`url(#sk-${accent})`}
//               />
//             </AreaChart>
...
// function ListPanel({
//   title, subtitle, items, loading, onViewAll, typeStyle,
//   emptyText, emptyIcon, avatarClass, icon, href,
// }) {
//   return (
//     <Panel
//       title={title}
...
//                 {it.href && (
//                   <Link
//                     href={it.href.startsWith("/dashboard") ? it.href : `/dashboard${it.href}`}
//                     className="shrink-0 rounded-lg bg-white px-2 py-1 text-[10px] font-black uppercase tracking-wide text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-900 hover:text-white"
//                   >
//                     Open
//                   </Link>
...
//             label="Approvals"
//             sub={`${pending.total ?? 0} pending`}
//             href="/dashboard/approval_config"
//             tone="amber"
//           />
//           <QuickAction
//             icon={Timer}
...
import {
  Users, UserCheck2, UserX, CalendarDays, Clock, AlertTriangle,
  Building2, Briefcase, ClipboardList, Cake, RefreshCw, TrendingUp,
  Sun, X, Printer, Zap, CircleDot, Award, Bell,
  ChevronRight, Sparkles, AlertCircle, UserPlus, Timer, Wallet,
  FileText, Target, Activity, Megaphone, Star, Coffee,
  ArrowUpRight, ArrowDownRight, CheckCircle2, Calendar as CalIcon,
  Rocket, Moon, Eye,
} from "lucide-react";
import {
...
function readRoleFromToken() {
  try {
    ...
    const r = String(payload.role || payload.user_role || payload.approle || "").toLowerCase();
    return r.includes("admin") ? "admin" : "employee";
  } catch {
    return "employee";
  }
}
...
function KpiCard({
  ...
  const accents = {
    red: { bg: "from-red-50 via-white to-white", border: "border-red-100/80", icon: "bg-red-100 text-[#E42527]", label: "text-[#E42527]", glow: "group-hover:shadow-red-100" },
    green: { bg: "from-emerald-50 via-white to-white", border: "border-emerald-100/80", icon: "bg-emerald-100 text-emerald-600", label: "text-emerald-600", glow: "group-hover:shadow-emerald-100" },
    amber: { bg: "from-amber-50 via-white to-white", border: "border-amber-100/80", icon: "bg-amber-100 text-amber-600", label: "text-amber-600", glow: "group-hover:shadow-amber-100" },
    blue: { bg: "from-sky-50 via-white to-white", border: "border-sky-100/80", icon: "bg-sky-100 text-sky-600", label: "text-sky-600", glow: "group-hover:shadow-sky-100" },
    slate: { bg: "from-slate-50 via-white to-white", border: "border-slate-200", icon: "bg-slate-100 text-slate-700", label: "text-slate-600", glow: "group-hover:shadow-slate-200" },
    violet: { bg: "from-violet-50 via-white to-white", border: "border-violet-100/80", icon: "bg-violet-100 text-violet-600", label: "text-violet-600", glow: "group-hover:shadow-violet-100" },
    rose: { bg: "from-rose-50 via-white to-white", border: "border-rose-100/80", icon: "bg-rose-100 text-rose-600", label: "text-rose-600", glow: "group-hover:shadow-rose-100" },
    teal: { bg: "from-teal-50 via-white to-white", border: "border-teal-100/80", icon: "bg-teal-100 text-teal-600", label: "text-teal-600", glow: "group-hover:shadow-teal-100" },
  };
  const a = accents[accent] || accents.slate;
  ...
  const inner = (
    <div className="flex h-full flex-col justify-between">
      <div>
        <div className="flex items-start justify-between gap-2">
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${a.icon} shadow-sm`}>
            <Icon className="h-5 w-5" />
          </div>
          {delta != null && (
            <span className={`inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-[10px] font-bold ${delta > 0 ? "bg-emerald-100 text-emerald-700" : delta < 0 ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-500"}`}>
              {delta > 0 ? <ArrowUpRight className="h-3 w-3" /> : delta < 0 ? <ArrowDownRight className="h-3 w-3" /> : null}
              {delta > 0 ? `+${delta}` : delta === 0 ? "—" : delta}
            </span>
          )}
        </div>
        <p className={`mt-3 text-[10px] font-bold uppercase tracking-widest ${a.label}`}>{label}</p>
        <div className="mt-1 flex items-baseline gap-2">
          <p className="text-3xl font-extrabold tracking-tight text-slate-900 tabular-nums">{value ?? 0}</p>
        </div>
  ...
  return (
    <button type="button" onClick={onClick} disabled={!onClick} className={`${baseClass} ${onClick ? "cursor-pointer" : "cursor-default"}`}>
      {inner}
    </button>
  );
}

function Panel({ title, subtitle, right, children, className = "", icon: Icon, iconClass }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm transition-all hover:shadow-md ${className}`}>
      <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div className="flex items-center gap-3">
          {Icon && (
            <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${iconClass || "bg-slate-100 text-slate-700"}`}>
              <Icon className="h-4.5 w-4.5" />
            </div>
          )}
          <div>
...
function QuickAction({ icon: Icon, label, href, tone = "slate", sub }) {
  ...
  const t = tones[tone] || tones.slate;
  return (
    <Link href={href} className={`group flex items-center gap-3 rounded-2xl p-3.5 ring-1 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${t}`}>
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white shadow-sm ring-1 ring-black/5">
        <Icon className="h-5 w-5" />
      </div>
      <div className="min-w-0 flex-1">
...
/* ═══════════════════════════════════════════════════════════════════════
   HERO (with role badge + view-as dropdown)
   ═══════════════════════════════════════════════════════════════════════ */

function Hero({
  summary, loading, onRefresh, refreshing, autoRefresh, setAutoRefresh,
  lastUpdated, headcount, att, deltas, onLeaveCount, absentNoLeaveCount,
  isAdmin, employees, viewAsEmp, setViewAsEmp,
}) {
  const GreetIcon = greetingIcon();
  const activeStaff = headcount.active || 0;
  const presentTotal = (att.present || 0) + (att.wfh || 0) + (att.on_duty || 0);
  const attendancePct = activeStaff ? Math.round((presentTotal / activeStaff) * 100) : 0;

  const selectedEmp = employees.find((e) => e.employee_id === viewAsEmp);

  const heroStats = [
    { label: isAdmin && !viewAsEmp ? "Active staff" : "Status", value: activeStaff, sub: `${headcount.total ?? 0} total`, delta: deltas.active, color: "text-white", icon: Users },
    { label: "Present today", value: att.present ?? 0, sub: `${attendancePct}% attendance`, delta: deltas.present, color: "text-emerald-300", icon: UserCheck2 },
    { label: "On leave", value: onLeaveCount, sub: `${att.half_day ?? 0} half-day`, delta: deltas.on_leave, color: "text-sky-300", icon: CalendarDays },
    { label: "Absent", value: absentNoLeaveCount, sub: `${att.late ?? 0} late arrivals`, delta: deltas.absent, color: "text-red-300", icon: UserX },
  ];
      ...
      <div className="relative">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black tracking-tight sm:text-4xl">
              {isAdmin && !viewAsEmp ? "HR Overview" : "My Dashboard"}
            </h1>
            <p className="mt-1.5 text-sm font-medium text-slate-300">
              {isAdmin && !viewAsEmp
                ? "Real-time headcount, attendance & workforce pulse"
                : selectedEmp
                ? `Viewing: ${selectedEmp.name} (${selectedEmp.employee_id})`
                : "Your personal attendance & leave overview"}
              {summary?.date ? ` · ${formatDate(summary.date)}` : ""}
            </p>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {lastUpdated && (
                <p className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-300 ring-1 ring-emerald-500/20">
                  <CircleDot className="h-2.5 w-2.5 animate-pulse" />
                  Live · {relativeTime(lastUpdated)}
                </p>
              )}
              <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${isAdmin ? "bg-amber-500/15 text-amber-300 ring-1 ring-amber-500/20" : "bg-sky-500/15 text-sky-300 ring-1 ring-sky-500/20"}`}>
          ...
          <div className="flex flex-wrap items-center gap-2 self-start">
            {isAdmin && (
              <div className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2 ring-1 ring-white/15 backdrop-blur">
                ...
                <select
                  value={viewAsEmp}
                  onChange={(e) => setViewAsEmp(e.target.value)}
                  className="max-w-[200px] bg-transparent text-xs font-bold uppercase tracking-wide text-white focus:outline-none"
                >
                  <option value="" className="text-slate-900">🏢 All Company</option>
                  {employees.map((e) => (
                    <option key={e.employee_id} value={e.employee_id} className="text-slate-900">
            ...
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-white/10 px-3 py-2.5 text-xs font-bold uppercase tracking-wide text-white ring-1 ring-white/15 backdrop-blur hover:bg-white/15">
              <input type="checkbox" checked={autoRefresh} onChange={(e) => setAutoRefresh(e.target.checked)} className="h-3.5 w-3.5 rounded border-white/40 accent-[#E42527]" />
              Auto
            </label>
            <button type="button" onClick={() => { try { window.print(); } catch {} }} className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2.5 text-xs font-bold uppercase tracking-wide text-white ring-1 ring-white/15 backdrop-blur hover:bg-white/15">
              <Printer className="h-3.5 w-3.5" /> Print
            </button>
            <button type="button" onClick={onRefresh} disabled={refreshing} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-bold uppercase tracking-wide text-slate-900 shadow-lg hover:bg-slate-100 disabled:opacity-60">
        ...
        {!loading && summary && (
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {heroStats.map((s) => {
              const Icon = s.icon;
              return (
                <div key={s.label} className="group relative overflow-hidden rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur-sm transition hover:bg-white/10">
                  <div className="flex items-center justify-between">
                    <Icon className={`h-4 w-4 ${s.color}`} />
                    {s.delta != null && s.delta !== 0 && (
                      <span className={`text-[10px] font-bold ${s.delta > 0 ? "text-emerald-400" : "text-red-400"}`}>
                        {s.delta > 0 ? "▲" : "▼"} {Math.abs(s.delta)}
                      </span>
                    )}
                  </div>
                  <p className="mt-2.5 text-2xl font-black tabular-nums">{s.value}</p>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{s.label}</p>
                  <p className="mt-0.5 text-[10px] font-medium text-slate-500">{s.sub}</p>
                </div>
              );
            })}
          </div>
...
function MyAttendanceCard({ data, loading }) {
  if (loading) {
    return (
      <Panel title="My day" subtitle="Loading..." icon={Timer} iconClass="bg-sky-50 text-sky-600">
        <div className="h-44 animate-pulse rounded-xl bg-slate-100" />
      </Panel>
    );
  }
  ...
  return (
    <Panel
      ...
      iconClass="bg-sky-50 text-sky-600"
      right={
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${isPunchedIn ? "bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200" : lastOut ? "bg-blue-100 text-blue-700 ring-1 ring-blue-200" : "bg-amber-100 text-amber-700 ring-1 ring-amber-200"}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${isPunchedIn ? "bg-emerald-500 animate-pulse" : lastOut ? "bg-blue-500" : "bg-amber-500"}`} />
          {isPunchedIn ? "Working" : lastOut ? "Completed" : "Not started"}
        </span>
      }
    >
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-emerald-50/70 px-3 py-3 ring-1 ring-emerald-100">
            <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-700">Punch in</p>
            <p className="mt-1 text-lg font-black text-slate-900 tabular-nums">{fmt(firstIn)}</p>
          </div>
          <div className="rounded-xl bg-red-50/70 px-3 py-3 ring-1 ring-red-100">
            <p className="text-[10px] font-bold uppercase tracking-widest text-red-700">Punch out</p>
            <p className="mt-1 text-lg font-black text-slate-900 tabular-nums">{fmt(lastOut)}</p>
          </div>
        </div>

        <div className="rounded-xl bg-slate-50 p-3.5">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold uppercase tracking-wide text-slate-500">Work duration</span>
            <span className="font-black text-slate-900 tabular-nums">
              {minutesToHhMm(workMinutes)}
              <span className="ml-1 text-[10px] font-medium text-slate-400">/ 8h</span>
            </span>
          </div>
          <div className="mt-2">
            <ProgressBar value={workMinutes} max={targetMinutes} color={progressPct >= 100 ? "#10b981" : progressPct >= 50 ? "#f59e0b" : "#E42527"} />
          </div>
        </div>
        ...
        <Link href="/dashboard/attendance/today" className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 py-2.5 text-xs font-bold uppercase tracking-wide text-white shadow-md hover:bg-slate-800">
          <Clock className="h-3.5 w-3.5" />
          {isPunchedIn ? "Punch Out" : lastOut ? "View Full Log" : "Punch In Now"}
        </Link>
      </div>
    </Panel>
  );
...
function MyLeaveCard({ balances, loading }) {
  if (loading) {
    return (
      <Panel title="Leave balance" subtitle="Loading..." icon={CalendarDays} iconClass="bg-violet-50 text-violet-600">
        <div className="h-44 animate-pulse rounded-xl bg-slate-100" />
      </Panel>
    );
  }
  ...
  if (!balances || balances.length === 0) {
    return (
      <Panel title="Leave balance" subtitle="Your leave summary" icon={CalendarDays} iconClass="bg-violet-50 text-violet-600">
        <Empty text="No leave balances configured" icon={CalendarDays} />
      </Panel>
    );
  }

  const totalAvailable = balances.reduce((s, b) => s + (Number(b.leaves_remaining) || 0), 0);
  const totalAllocated = balances.reduce((s, b) => s + (Number(b.total_leaves) || 0), 0);
  const totalUsed = balances.reduce((s, b) => s + (Number(b.leaves_taken) || 0), 0);
  ...
  return (
    <Panel
      ...
      icon={CalendarDays}
      iconClass="bg-violet-50 text-violet-600"
      right={<Link href="/dashboard/leave/leave-balances" className="text-[10px] font-bold uppercase tracking-wide text-[#E42527] hover:underline">Manage</Link>}
    >
      <div className="space-y-3">
        <div className="rounded-xl bg-gradient-to-br from-violet-50 to-white p-3.5 ring-1 ring-violet-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-violet-700">Available</p>
              <p className="mt-0.5 text-2xl font-black text-slate-900 tabular-nums">
                {totalAvailable.toFixed(1)}
            ...
            <div className="text-right">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Used</p>
              <p className="mt-0.5 text-sm font-bold text-slate-700 tabular-nums">
                {totalUsed.toFixed(1)} / {totalAllocated.toFixed(1)}
              </p>
            </div>
          ...
          <div className="mt-2.5">
            <ProgressBar value={totalUsed} max={totalAllocated} color="#8b5cf6" />
          </div>
        </div>
...
function AnnouncementsCard({ announcements, loading }) {
  const priorityStyles = {
    high: { border: "border-l-red-500", bg: "bg-red-50/40", badge: "bg-red-100 text-red-700" },
    normal: { border: "border-l-sky-500", bg: "bg-sky-50/40", badge: "bg-sky-100 text-sky-700" },
    low: { border: "border-l-slate-400", bg: "bg-slate-50", badge: "bg-slate-100 text-slate-600" },
  };
  ...
  return (
    <Panel
      ...
      icon={Megaphone}
      iconClass="bg-amber-50 text-amber-600"
      right={<Link href="/dashboard/announcements" className="text-[10px] font-bold uppercase tracking-wide text-[#E42527] hover:underline">View all</Link>}
    >
      {loading ? (
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
        ...
        <div className="space-y-2.5">
          {announcements.slice(0, 3).map((a, i) => {
            ...
            return (
              <div key={a.id || i} className={`rounded-lg border-l-4 ${style.border} ${style.bg} px-3 py-2.5`}>
                <div className="flex items-start justify-between gap-2">
                  <p className="line-clamp-1 text-[13px] font-bold text-slate-900">{a.title}</p>
                  {a.priority && a.priority !== "low" && (
                    <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-black uppercase ${style.badge}`}>{a.priority}</span>
                  )}
                </div>
                {a.body && <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-slate-600">{a.body}</p>}
                <div className="mt-1.5 flex items-center gap-1.5 text-[10px] font-medium text-slate-400">
                  <span>{a.posted_by || "HR Team"}</span>
                  <span>·</span>
                  <span>{relativeTime(a.posted_at ? new Date(a.posted_at).getTime() : null)}</span>
                </div>
              </div>
            );
          })}
...
function AttendanceTrendChart({ data, loading }) {
  return (
    <Panel
      ...
      right={
        <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wide">
          <span className="flex items-center gap-1.5 text-emerald-600"><span className="h-2 w-2 rounded-full bg-emerald-500" />Present</span>
          <span className="flex items-center gap-1.5 text-[#E42527]"><span className="h-2 w-2 rounded-full bg-[#E42527]" />Absent</span>
          <span className="flex items-center gap-1.5 text-indigo-500"><span className="h-2 w-2 rounded-full bg-indigo-500" />Leave</span>
        </div>
      }
    >
      {loading ? (
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.08)" }} />
              <Area type="monotone" dataKey="present" name="Present" stroke="#10b981" fill="url(#gradPresent)" strokeWidth={2.5} />
              <Area type="monotone" dataKey="absent" name="Absent" stroke="#E42527" fill="url(#gradAbsent)" strokeWidth={2} />
              <Area type="monotone" dataKey="leave" name="On leave" stroke="#6366f1" fill="transparent" strokeWidth={2} strokeDasharray="5 4" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
...
function AttendanceRateRing({ percent, present, total, loading }) {
  ...
  return (
    <Panel title="Attendance rate" subtitle="Present + WFH + On-duty" icon={Activity} iconClass="bg-blue-50 text-blue-600" className="lg:col-span-3">
      {loading ? (
        <div className="h-[260px] animate-pulse rounded-xl bg-slate-100" />
      ) : (
        <div className="flex flex-col items-center">
          <div className="relative flex h-[200px] w-full items-center justify-center">
            ...
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              <p className="text-4xl font-black text-slate-900 tabular-nums">{pct}%</p>
              <p className="mt-0.5 text-[10px] font-bold uppercase tracking-widest" style={{ color }}>{status}</p>
            </div>
          </div>
          <div className="mt-3 grid w-full grid-cols-2 gap-2">
            <div className="rounded-xl bg-slate-50 px-3 py-2.5 text-center">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Present</p>
              <p className="mt-0.5 text-lg font-black text-emerald-600 tabular-nums">{present ?? 0}</p>
            </div>
            <div className="rounded-xl bg-slate-50 px-3 py-2.5 text-center">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Total</p>
              <p className="mt-0.5 text-lg font-black text-slate-900 tabular-nums">{total ?? 0}</p>
            </div>
          </div>
        </div>
      )}
...
function DeptPresenceBar({ data, loading }) {
  return (
    <Panel title="Department presence" subtitle="In office today" icon={Building2} iconClass="bg-rose-50 text-rose-600" className="lg:col-span-3">
      {loading ? (
        <div className="h-[260px] animate-pulse rounded-xl bg-slate-100" />
      ) : !data || data.length === 0 ? (
        <Empty text="No department data" icon={Building2} />
        ...
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ left: 4, right: 12, top: 4, bottom: 4 }}>
              ...
              <YAxis type="category" dataKey="department" width={90} tick={{ fontSize: 10, fill: "#64748b", fontWeight: 600 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 12 }} cursor={{ fill: "rgba(228,37,39,0.04)" }} />
              <Bar dataKey="present" fill="#E42527" radius={[0, 8, 8, 0]} barSize={14} />
            </BarChart>
          </ResponsiveContainer>
        </div>
...
function DeptHeadcountPie({ data, loading }) {
  ...
  return (
    <Panel title="Headcount by department" subtitle={`${total} employees`} icon={Users} iconClass="bg-violet-50 text-violet-600" className="lg:col-span-6">
      {loading ? (
        <div className="h-[300px] animate-pulse rounded-xl bg-slate-100" />
      ) : !data || data.length === 0 ? (
        <Empty text="No department data" icon={Building2} />
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={95} paddingAngle={3} dataKey="count" nameKey="department">
                {data.map((_, i) => (
                  <Cell key={i} fill={DEPT_COLORS[i % DEPT_COLORS.length]} stroke="#fff" strokeWidth={2} />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{ borderRadius: 12, border: "1px solid #e2e8f0", fontSize: 12 }}
                formatter={(v, n, p) => [`${v} employees (${Math.round((v / total) * 100)}%)`, p.payload.department]}
              />
              <Legend iconType="circle" wrapperStyle={{ fontSize: 11, fontWeight: 600 }} formatter={(value, entry) => `${value} (${entry?.payload?.count || 0})`} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      )}
...
function PayrollCard({ data, loading }) {
  ...
  return (
    <Panel
      ...
      iconClass="bg-emerald-50 text-emerald-600"
      right={
        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ring-1 ${status.tone}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${status.dot}`} />
          {status.label}
        </span>
      }
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="mt-0.5 text-sm font-bold text-slate-900">{data.period || "Current month"}</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-white p-3 ring-1 ring-emerald-100">
              <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-700">Net pay</p>
              <p className="mt-0.5 text-base font-black text-slate-900 tabular-nums">₹{(data.total_net_pay || 0).toLocaleString("en-IN")}</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-sky-50 to-white p-3 ring-1 ring-sky-100">
              <p className="text-[10px] font-bold uppercase tracking-widest text-sky-700">Employees</p>
              <p className="mt-0.5 text-base font-black text-slate-900 tabular-nums">{data.total_employees || 0}</p>
            </div>
          </div>
        </div>
      )}
...
function RecruitmentCard({ data, loading }) {
  return (
    <Panel title="Recruitment" subtitle="Open roles & pipeline" icon={Target} iconClass="bg-violet-50 text-violet-600">
      {loading ? (
        <div className="h-32 animate-pulse rounded-xl bg-slate-100" />
      ) : !data ? (
        <Empty text="No recruitment data" icon={Target} />
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-xl bg-gradient-to-br from-violet-50 to-white p-3 ring-1 ring-violet-100">
              <p className="text-[10px] font-bold uppercase tracking-widest text-violet-700">Open roles</p>
              <p className="mt-0.5 text-2xl font-black text-violet-900 tabular-nums">{data.open_positions || 0}</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-sky-50 to-white p-3 ring-1 ring-sky-100">
              <p className="text-[10px] font-bold uppercase tracking-widest text-sky-700">Candidates</p>
              <p className="mt-0.5 text-2xl font-black text-sky-900 tabular-nums">{data.in_pipeline || 0}</p>
            </div>
          </div>
          {(data.stages || []).length > 0 && (
            <div className="space-y-1.5">
              {data.stages.slice(0, 3).map((s, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-1.5 text-[11px] font-medium">
                  <span className="text-slate-600">{s.name}</span>
                  <span className="font-black text-slate-900">{s.count}</span>
                </div>
              ))}
...
function DocumentExpiryCard({ docs, loading }) {
  ...
  return (
    <Panel
      ...
      icon={FileText}
      iconClass="bg-amber-50 text-amber-600"
      right={docs?.length > 0 && <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-black uppercase text-red-700">{docs.length} due</span>}
    >
      {loading ? (
        <div className="h-32 animate-pulse rounded-xl bg-slate-100" />
      ) : !docs || docs.length === 0 ? (
        ...
        <div className="space-y-2">
          {docs.slice(0, 3).map((d, i) => {
            const daysLeft = d.days_left ?? 0;
            const urgency = daysLeft <= 7 ? "critical" : daysLeft <= 15 ? "warning" : "normal";
            return (
              <div key={d.id || i} className={`rounded-lg border-l-4 px-3 py-2 ${urgencyStyles[urgency]}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="truncate text-[11px] font-bold text-slate-900">{d.document_type || "Document"}</p>
                    <p className="mt-0.5 truncate text-[10px] font-medium text-slate-600">{d.employee_name || "—"}</p>
                    <p className="mt-0.5 text-[10px] text-slate-400">{formatDate(d.expiry_date)}</p>
                  </div>
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black ${urgency === "critical" ? "bg-red-100 text-red-700" : urgency === "warning" ? "bg-amber-100 text-amber-700" : "bg-sky-100 text-sky-700"}`}>
                    {daysLeft}d
                  </span>
                </div>
              </div>
...
function WorkingHoursCard({ data, loading }) {
  return (
    <Panel title="Working hours" subtitle="Average this week" icon={Clock} iconClass="bg-teal-50 text-teal-600">
      {loading ? (
        <div className="h-32 animate-pulse rounded-xl bg-slate-100" />
      ) : !data ? (
        <Empty text="No data available" icon={Clock} />
        ...
        <div className="space-y-3">
          <div className="rounded-xl bg-gradient-to-br from-teal-50 to-white p-4 ring-1 ring-teal-100">
            <p className="text-[10px] font-bold uppercase tracking-widest text-teal-700">Daily average</p>
            <p className="mt-1 text-3xl font-black text-slate-900 tabular-nums">{data.avg_hours || "0h 0m"}</p>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-lg bg-slate-50 px-2 py-2 text-center">
              <p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Min</p>
              <p className="mt-0.5 text-xs font-black text-slate-900">{data.min || "—"}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-2 py-2 text-center">
              <p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Max</p>
              <p className="mt-0.5 text-xs font-black text-slate-900">{data.max || "—"}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-2 py-2 text-center">
              <p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Total</p>
              <p className="mt-0.5 text-xs font-black text-slate-900">{data.total || "—"}</p>
            </div>
          </div>
        </div>
      )}
...
function AttritionCard({ data, loading }) {
  const rate = data?.rate || 0;
  const health = rate <= 5 ? "Healthy" : rate <= 10 ? "Moderate" : "Concerning";
  const healthColor = rate <= 5 ? "text-emerald-600 bg-emerald-50 ring-emerald-100" : rate <= 10 ? "text-amber-600 bg-amber-50 ring-amber-100" : "text-red-600 bg-red-50 ring-red-100";
  ...
  return (
    <Panel
      ...
      icon={Activity}
      iconClass="bg-red-50 text-red-600"
      right={!loading && data && <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase ring-1 ${healthColor}`}>{health}</span>}
    >
      {loading ? (
        <div className="h-32 animate-pulse rounded-xl bg-slate-100" />
      ) : !data ? (
        ...
        <div className="space-y-3">
          <div className="flex items-baseline gap-2">
            <p className="text-4xl font-black text-red-600 tabular-nums">{rate}%</p>
            <span className="text-[10px] font-medium uppercase tracking-wide text-slate-400">monthly rate</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-lg bg-slate-50 px-3 py-2">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Exits</p>
              <p className="mt-0.5 text-lg font-black text-slate-900">{data.exits || 0}</p>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Active</p>
              <p className="mt-0.5 text-lg font-black text-slate-900">{data.total_active || 0}</p>
            </div>
          </div>
        </div>
      )}
...
function BirthdaysPanel({ birthdays, loading }) {
  return (
    <Panel title="Birthdays" subtitle={birthdays?.length ? `${birthdays.length} this week` : "Next 7 days"} icon={Cake} iconClass="bg-rose-50 text-rose-600">
      {loading ? (
        ...
        <div className="space-y-2">
          {birthdays.slice(0, 4).map((b, i) => (
            <div key={b.employee_id || i} className="group flex items-center gap-3 rounded-xl border border-rose-100 bg-gradient-to-r from-rose-50 to-white p-3 transition hover:border-rose-200 hover:shadow-sm">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#E42527] to-[#b91c1c] text-sm font-black text-white shadow-md shadow-red-200">
                {getInitials(b.name)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold text-slate-900">{b.name || b.employee_id}</p>
                <p className="mt-0.5 flex items-center gap-1 text-[10px] font-medium text-slate-500">
                  <Cake className="h-2.5 w-2.5" />{formatDate(b.birthday_on || b.dob)}
                </p>
              </div>
              <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wide ${b.days_until === 0 ? "bg-[#E42527] text-white shadow-sm" : b.days_until <= 2 ? "bg-rose-100 text-rose-700" : "bg-slate-100 text-slate-600"}`}>
...
function AnniversariesPanel({ anniversaries, loading }) {
  return (
    <Panel title="Work anniversaries" subtitle={anniversaries?.length ? `${anniversaries.length} upcoming` : "Next 30 days"} icon={Award} iconClass="bg-violet-50 text-violet-600">
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-lg bg-slate-100" />)}</div>
      ) : !anniversaries || anniversaries.length === 0 ? (
        <Empty text="No anniversaries this month" icon={Award} />
        ...
        <div className="space-y-2">
          {anniversaries.slice(0, 4).map((a, i) => (
            <div key={a.employee_id || i} className="group flex items-center gap-3 rounded-xl border border-violet-100 bg-gradient-to-r from-violet-50 to-white p-3 transition hover:border-violet-200 hover:shadow-sm">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-violet-800 text-sm font-black text-white shadow-md shadow-violet-200">
                {getInitials(a.name)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold text-slate-900">{a.name || a.employee_id}</p>
                <div className="mt-0.5 flex items-center gap-1.5">
                  <span className="inline-flex items-center gap-0.5 rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-black uppercase text-amber-700">
                    <Star className="h-2.5 w-2.5" />{a.years} {a.years === 1 ? "year" : "years"}
                  </span>
                  {a.designation && <span className="truncate text-[10px] font-medium text-slate-500">{a.designation}</span>}
                </div>
              </div>
              <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wide ${a.days_until === 0 ? "bg-violet-600 text-white shadow-sm" : "bg-violet-100 text-violet-700"}`}>
                {a.days_until === 0 ? "Today" : `${a.days_until}d`}
              </span>
            </div>
          ))}
...
function EventsPanel({ events, loading }) {
  ...
  return (
    <Panel
      title="This week"
      subtitle={events?.length ? `${events.length} upcoming events` : "Events & reminders"}
      icon={CalIcon}
      iconClass="bg-indigo-50 text-indigo-600"
      className="lg:col-span-6"
    >
      {loading ? (
        ...
        <div className="max-h-[340px] space-y-2 overflow-y-auto pr-1">
          {events.slice(0, 8).map((e, i) => {
            ...
            const style = eventColors[e.type] || eventColors.default;
            return (
              <div key={e.id || i} className="flex items-center gap-3 rounded-xl bg-slate-50 p-2.5 transition hover:bg-slate-100">
                <div className="flex h-11 w-11 shrink-0 flex-col items-center justify-center rounded-lg bg-white shadow-sm ring-1 ring-slate-200">
                  <span className="text-[9px] font-black uppercase tracking-wide text-slate-500">{date.month}</span>
                  <span className="text-base font-black leading-4 text-slate-900">{date.day}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[12px] font-bold text-slate-900">{style.icon} {e.title}</p>
                  <div className="mt-1 flex items-center gap-1.5">
                    <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide ring-1 ${style.bg} ${style.text} ${style.ring}`}>{e.type}</span>
                    {e.time && <span className="text-[10px] font-medium text-slate-500">{e.time}</span>}
                  </div>
                </div>
              </div>
            );
...
function ListPanel({ title, subtitle, items, loading, onViewAll, typeStyle, emptyText, emptyIcon, avatarClass, icon }) {
  return (
    <Panel
      title={title}
      subtitle={subtitle}
      icon={icon?.Icon}
      iconClass={icon?.className}
      right={items?.length > 0 && onViewAll && (
        <button onClick={onViewAll} className="text-[10px] font-bold uppercase tracking-wide text-[#E42527] hover:underline">View all</button>
      )}
    >
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-12 animate-pulse rounded-lg bg-slate-100" />)}</div>
      ) : !items || items.length === 0 ? (
        <Empty text={emptyText} icon={emptyIcon} />
      ) : (
        <div className="space-y-2">
          {items.slice(0, 4).map((row, i) => (
            <div key={row.employee_id || i} className="flex items-center gap-3 rounded-xl bg-slate-50 p-2.5">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[11px] font-black text-white ${avatarClass}`}>
                {getInitials(row.name || row.employee_id)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-bold text-slate-900">{row.name || row.employee_id}</p>
                <p className="mt-0.5 truncate text-[10px] font-medium text-slate-500">
                  {row.leave_type || row.reason || row.title || "—"}
                  {row.start_date && ` · ${formatDate(row.start_date)}`}
                </p>
              </div>
              {row.type && typeStyle && (
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wide ring-1 ${typeStyle(row.type)}`}>
                  {(row.type || "").replace(/_/g, " ")}
                </span>
              )}
            </div>
...
function HolidaysPanel({ holidays, loading }) {
  return (
    <Panel title="Upcoming holidays" subtitle={holidays?.length ? `${holidays.length} next 30 days` : "Next 30 days"} icon={Sun} iconClass="bg-amber-50 text-amber-600">
      {loading ? (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
          {holidays.slice(0, 8).map((h, i) => {
            const date = calendarParts(h.date);
            const daysAway = Math.ceil((new Date(h.date) - new Date()) / (1000 * 60 * 60 * 24));
            return (
              <div key={h.holiday_id || i} className="group flex items-center gap-3 rounded-xl border border-amber-100 bg-gradient-to-br from-amber-50/60 to-white p-3 transition hover:border-amber-200 hover:shadow-sm">
                <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-lg bg-white text-amber-700 shadow-sm ring-1 ring-amber-100">
                  <span className="text-[9px] font-black uppercase tracking-wide">{date.month}</span>
                  <span className="text-lg font-black leading-5">{date.day}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[12px] font-bold text-slate-900">{h.name || h.holiday_name}</p>
                  <div className="mt-0.5 flex items-center gap-1.5">
                    <span className="text-[10px] font-medium text-slate-500">{date.weekday}</span>
                    {daysAway > 0 && (
                      <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-black uppercase text-amber-700">
                        {daysAway === 1 ? "Tomorrow" : `in ${daysAway}d`}
                      </span>
...
function ActionRequiredPanel({ items, loading }) {
  ...
  return (
    <Panel
      ...
      icon={Sparkles}
      iconClass="bg-amber-50 text-amber-600"
      right={items?.length > 0 && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-black uppercase text-amber-700">{items.length} pending</span>}
    >
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-lg bg-slate-100" />)}</div>
      ) : !items || items.length === 0 ? (
        ...
        <ul className="space-y-2">
          {items.slice(0, 5).map((it, i) => {
            const p = priority[it.priority] || priority.low;
            return (
              <li key={it.id || i} className="flex items-start gap-3 rounded-xl bg-slate-50 p-3 transition hover:bg-slate-100">
                <div className="mt-1"><span className={`block h-2 w-2 rounded-full ${p.dot}`} /></div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <p className="line-clamp-1 text-[12px] font-bold text-slate-900">{it.title}</p>
                    <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide ring-1 ${p.bg} ${p.text} ${p.ring}`}>{it.priority || "low"}</span>
                  </div>
                  {it.description && <p className="mt-0.5 line-clamp-1 text-[10px] font-medium text-slate-500">{it.description}</p>}
                </div>
                {it.href && (
                  <Link href={it.href.startsWith("/dashboard") ? it.href : `/dashboard${it.href}`} className="shrink-0 rounded-lg bg-white px-2 py-1 text-[10px] font-black uppercase tracking-wide text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-900 hover:text-white">
                    Open
                  </Link>
                )}
              </li>
...
function TeamPanel({ team, loading }) {
  ...
  return (
    <Panel
      ...
      iconClass="bg-blue-50 text-blue-600"
      right={
        <span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ring-1 ${pct >= 75 ? "bg-emerald-100 text-emerald-700 ring-emerald-200" : pct >= 50 ? "bg-amber-100 text-amber-700 ring-amber-200" : "bg-red-100 text-red-700 ring-red-200"}`}>
          {pct}% present
        </span>
      }
    >
      {loading ? (
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-4 gap-2">
            {["present", "absent", "on_leave", "late"].map((k) => (
              <div key={k} className="rounded-xl bg-gradient-to-br from-slate-50 to-white p-2.5 text-center ring-1 ring-slate-100">
                <p className="text-[9px] font-black uppercase tracking-wide text-slate-600">{k.replace("_", " ")}</p>
                <p className="mt-0.5 text-lg font-black text-slate-900 tabular-nums">{team[k] ?? 0}</p>
              </div>
            ))}
          </div>
          {Array.isArray(team.members) && team.members.length > 0 && (
            <div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
              {team.members.slice(0, 6).map((m, i) => {
                const statusStyle = m.status === "present" ? "bg-emerald-500" : m.status === "absent" ? "bg-red-500" : m.status === "on_leave" ? "bg-sky-500" : "bg-slate-400";
                return (
                  <div key={m.employee_id || i} className="flex items-center gap-2 rounded-lg bg-slate-50 px-2.5 py-1.5">
                    <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${statusStyle}`} />
                    <span className="truncate text-[11px] font-medium text-slate-700">{m.name || m.employee_id}</span>
                  </div>
                );
              })}
            </div>
...
function RecentHiresPanel({ hires, loading }) {
  return (
    <Panel
      ...
      icon={UserPlus}
      iconClass="bg-emerald-50 text-emerald-600"
      right={hires?.length > 0 && <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black uppercase text-emerald-700">{hires.length} new</span>}
    >
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-lg bg-slate-100" />)}</div>
      ) : !hires || hires.length === 0 ? (
        ...
        <div className="space-y-2">
          {hires.slice(0, 5).map((h, i) => (
            <div key={h.employee_id || i} className="flex items-center gap-3 rounded-xl bg-emerald-50/40 p-2.5 ring-1 ring-emerald-100/60">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 text-[11px] font-black text-white shadow-sm">
                {getInitials(h.name)}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] font-bold text-slate-900">{h.name || h.employee_id}</p>
                <p className="mt-0.5 truncate text-[10px] font-medium text-slate-500">
                  {h.designation || "—"}{h.department && ` · ${h.department}`}
                </p>
              </div>
              {h.joining_date && (
                <span className="shrink-0 rounded-full bg-white px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-200">
...
export default function HrmsDashboardPage() {
  ...
  const [viewAsEmp, setViewAsEmp] = useState("");
  ...
  // ── Load employees list (admin only) ──
  useEffect(() => {
    if (role !== "admin") return;
    api.get("/api/v1/dashboard/employees-list")
      .then((r) => {
        const d = r.data?.employees ?? r.data?.data?.employees ?? [];
  ...
  const loadDashboard = useCallback(async ({ silent = false } = {}) => {
    ...
    const p = viewAsEmp ? { employee_id: viewAsEmp } : {};

    try {
      const results = await Promise.allSettled([
        api.get("/api/v1/dashboard/summary", { params: p }),
        api.get("/api/v1/dashboard/attendance-trend", { params: { days: 7, ...p } }),
        api.get("/api/v1/dashboard/pending-approvals", { params: { limit: 8, ...p } }),
        api.get("/api/v1/dashboard/today-people-status", { params: p }),
        api.get("/api/v1/dashboard/action-required"),
        api.get("/api/v1/dashboard/recent-hires", { params: { days: 30, ...p } }),
        api.get("/api/v1/dashboard/my-team-today"),
        api.get("/api/v1/dashboard/my-today"),
        api.get("/api/v1/dashboard/my-leave-balance"),
        api.get("/api/v1/dashboard/announcements", { params: { limit: 5 } }),
        api.get("/api/v1/dashboard/upcoming-events", { params: { days: 7, ...p } }),
        api.get("/api/v1/dashboard/dept-headcount"),
        api.get("/api/v1/dashboard/payroll-status"),
        api.get("/api/v1/dashboard/document-expiries", { params: { days: 30, ...p } }),
        api.get("/api/v1/dashboard/recruitment-pipeline"),
        api.get("/api/v1/dashboard/working-hours-avg", { params: p }),
        api.get("/api/v1/dashboard/attrition"),
        api.get("/api/v1/dashboard/anniversaries", { params: { days: 30, ...p } }),
        api.get("/api/v1/dashboard/upcoming-events", { params: { days: 30, ...p } }),
      ]);

      if (myReqId !== reqIdRef.current) return;

      const safe = (r) => r.status === "fulfilled" ? (r.value?.data ?? r.value) : null;

      const [sumR, trendR, pendR, peopleR, actionsR, hiresR, teamR, myTodayR, myBalR, annR, eventsR, deptR, payrollR, docsR, recruitR, hoursR, attrR, annivR, events30R] = results;
      ...
      const h = safe(hiresR);
      setRecentHires(Array.isArray(h?.employees) ? h.employees : Array.isArray(h?.items) ? h.items : []);
      ...
      const de = safe(deptR);
      setDeptHeadcount(Array.isArray(de?.departments) ? de.departments : Array.isArray(de) ? de : []);
  ...
  useEffect(() => {
    if (!autoRefresh || role === null) return;
    const t = setInterval(() => loadDashboard({ silent: true }), AUTO_REFRESH_MS);
    return () => clearInterval(t);
  }, [autoRefresh, loadDashboard, role]);
  ...
  const trendData = useMemo(() => {
    return (trend || []).map((row) => {
      ...
      return { day, present: row.present || 0, absent: row.absent || 0, leave: row.on_leave || 0 };
    });
  }, [trend]);
  ...
  const activeStaff = headcount.active || 0;
  const presentTotal = (att.present || 0) + (att.wfh || 0) + (att.on_duty || 0);
  const attendancePercent = activeStaff ? Math.round((presentTotal / activeStaff) * 100) : presentRate ?? 0;

  const typeStyle = (type) => {
    const t = (type || "").toLowerCase();
    if (t === "leave") return "bg-sky-100 text-sky-700 ring-sky-200";
    if (t === "regularization") return "bg-violet-100 text-violet-700 ring-violet-200";
    if (t === "special_request") return "bg-amber-100 text-amber-700 ring-amber-200";
    return "bg-slate-100 text-slate-700 ring-slate-200";
  };

  const modalTitle = listModal === "leave" ? "On leave today" : listModal === "absent" ? "Absent without leave" : "";
  const modalList = listModal === "leave" ? onLeaveList : listModal === "absent" ? absentList : [];

  // ── Widget visibility flags ──
  const showCompanyView = isAdmin && !viewAsEmp;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-6">
        <div className="max-w-md rounded-3xl border border-red-200 bg-white p-8 text-center shadow-xl">
          ...
          <h2 className="mt-5 text-xl font-black text-slate-900">Dashboard load failed</h2>
          <p className="mt-2 text-sm font-medium text-slate-600">{error}</p>
          <button onClick={() => loadDashboard()} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#E42527] px-6 py-3 text-sm font-bold uppercase tracking-wide text-white shadow-lg hover:bg-[#c91f21]">
  ...
  if (role === null) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-slate-200 border-t-[#E42527]" />
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-[1600px] space-y-5 px-4 py-5 sm:px-6 lg:px-8">
        ...
        {error && summary && (
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
            ...
            <button onClick={() => loadDashboard()} className="rounded-lg border border-amber-300 bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide text-amber-800 hover:bg-amber-100">
              Retry
            </button>
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {isAdmin && showCompanyView && (
            <>
              <QuickAction icon={Zap} label="Run Payroll" sub="Process this month" href="/dashboard/payroll/run" tone="red" />
              <QuickAction icon={UserPlus} label="Add Employee" sub="Onboard new hire" href="/dashboard/employees" tone="green" />
            </>
          )}
          <QuickAction
            icon={ClipboardList}
            label={isAdmin ? "Approvals" : "My Requests"}
            sub={`${pending.total ?? 0} pending`}
            href={isAdmin ? "/dashboard/approval_config" : "/dashboard/leave/my-requests"}
            tone="amber"
          />
          <QuickAction icon={Timer} label="My Attendance" sub="Punch & timesheet" href="/dashboard/attendance/today" tone="blue" />
        </div>

        {/* PERSONAL ROW — always visible */}
        <div className="grid gap-4 lg:grid-cols-3">
          <MyAttendanceCard data={myToday} loading={loading && !myToday} />
          <MyLeaveCard balances={myBalances} loading={loading && !myBalances.length} />
          <AnnouncementsCard announcements={announcements} loading={loading && !announcements.length} />
        </div>
        ...
        {loading && !summary ? (
          ...
          <>
            {/* PRIMARY KPIs */}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              <KpiCard icon={Users} label={showCompanyView ? "Active staff" : "Status"} value={activeStaff} hint={`${headcount.total ?? 0} total`} accent="slate" delta={deltas.active} href={showCompanyView ? "/dashboard/employees" : undefined} />
              <KpiCard icon={UserCheck2} label="Present" value={att.present ?? 0} hint={`${att.late ?? 0} late arrivals`} accent="green" delta={deltas.present} />
              <KpiCard icon={CalendarDays} label="On leave" value={onLeaveCount} hint="Approved leave today" accent="blue" delta={deltas.on_leave} onClick={showCompanyView ? () => setListModal("leave") : undefined} />
              <KpiCard icon={UserX} label="Absent" value={absentNoLeaveCount} hint="No punch, no leave" accent="red" delta={deltas.absent} onClick={showCompanyView ? () => setListModal("absent") : undefined} />
              <KpiCard icon={ClipboardList} label="Approvals" value={pending.total ?? 0} hint={`${pending.leaves ?? 0} leave requests`} accent="amber" href={isAdmin ? "/dashboard/approval_config" : "/dashboard/leave/my-requests"} />
              <KpiCard icon={Briefcase} label="New joiners" value={headcount.new_joiners_this_month ?? 0} hint="This month" accent="violet" href={showCompanyView ? "/dashboard/employees" : undefined} />
            </div>

            {/* SECONDARY KPIs — admin only (hide when viewing as specific employee) */}
            {isAdmin && !viewAsEmp && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <KpiCard icon={Clock} label="Work from home" value={att.wfh ?? 0} hint="Today's remote staff" accent="blue" />
                <KpiCard icon={Bell} label="Unread alerts" value={summary?.unread_alerts ?? 0} hint={(summary?.unread_alerts || 0) > 0 ? "Requires attention" : "No pending alerts"} accent={(summary?.unread_alerts || 0) > 0 ? "amber" : "slate"} href="/dashboard/attendance/alerts" />
                <KpiCard icon={Building2} label="Notice period" value={headcount.notice_period ?? 0} hint="Employees serving notice" accent="slate" href="/dashboard/employees" />
                <KpiCard icon={Rocket} label="Onboarding" value={summary?.onboarding_candidates ?? 0} hint={presentRate != null ? `Attendance ${presentRate}%` : "Pipeline active"} accent="violet" href="/dashboard/Onboarding" />
              </div>
            )}
            ...
            <div className="grid gap-4 lg:grid-cols-12">
              <AttendanceTrendChart data={trendData} loading={loading} />
              <AttendanceRateRing percent={attendancePercent} present={presentTotal} total={activeStaff} loading={loading} />
              {isAdmin && !viewAsEmp && <DeptPresenceBar data={deptPresent} loading={loading} />}
            </div>

            {/* CHARTS ROW 2 */}
            <div className="grid gap-4 lg:grid-cols-12">
              {isAdmin && !viewAsEmp && <DeptHeadcountPie data={deptHeadcount} loading={loading && !deptHeadcount.length} />}
              <EventsPanel events={events} loading={loading && !events.length} />
            </div>

            {/* OPERATIONS — admin only, hide when viewing single employee */}
            {isAdmin && !viewAsEmp && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <PayrollCard data={payroll} loading={loading && !payroll} />
                <RecruitmentCard data={recruitment} loading={loading && !recruitment} />
                <DocumentExpiryCard docs={docExpiries} loading={loading && !docExpiries.length} />
                <WorkingHoursCard data={workingHours} loading={loading && !workingHours} />
              </div>
            )}
            ...
            {/* CELEBRATIONS */}
            <div className="grid gap-4 lg:grid-cols-3">
              <BirthdaysPanel birthdays={birthdays} loading={loading && !birthdays.length} />
              <AnniversariesPanel anniversaries={anniversaries} loading={loading && !anniversaries.length} />
              {isAdmin && !viewAsEmp && <AttritionCard data={attrition} loading={loading && !attrition} />}
            </div>

            {/* ACTION / TEAM / HIRES — admin only, hide when viewing single employee */}
            {isAdmin && !viewAsEmp && (
              <div className="grid gap-4 lg:grid-cols-3">
                <ActionRequiredPanel items={actionItems} loading={loading && !actionItems.length} />
                {myTeam && myTeam.total > 0 && <TeamPanel team={myTeam} loading={loading} />}
                <RecentHiresPanel hires={recentHires} loading={loading && !recentHires.length} />
              </div>
            )}

            {/* LISTS — admin company view only */}
            {isAdmin && !viewAsEmp && (
              <div className="grid gap-4 lg:grid-cols-3">
                <ListPanel
                  title="On leave today"
                ...
                <ListPanel
                  ...
                  emptyIcon={CheckCircle2}
                  avatarClass="bg-gradient-to-br from-amber-500 to-amber-700"
                  icon={{ Icon: ClipboardList, className: "bg-amber-50 text-amber-600" }}
                  typeStyle={typeStyle}
                />
              </div>
            )}
            ...
            {/* HOLIDAYS — always */}
            <HolidaysPanel holidays={holidays} loading={loading && !holidays.length} />
          </>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              ...
              <button type="button" onClick={() => setListModal(null)} className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {modalList.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-100">
                  {modalList.map((row, i) => (
                    <li key={row.employee_id || i} className="py-3.5">
                      <div className="flex items-start gap-3">
                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-black text-white ${listModal === "leave" ? "bg-gradient-to-br from-sky-500 to-sky-700" : "bg-gradient-to-br from-[#E42527] to-[#b91c1c]"}`}>
                          {getInitials(row.name || row.employee_id)}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-bold text-slate-900">{row.name || row.employee_id}</p>
                          <p className="text-[10px] font-medium uppercase tracking-wide text-slate-400">{row.employee_id}</p>
                          {listModal === "leave" ? (
                            <p className="mt-1 text-xs font-medium text-sky-700">
                              {row.leave_type || "Leave"}
                              {row.start_date ? ` · ${formatDate(row.start_date)}` : ""}
                            ...
                            <p className="mt-1 text-xs font-medium text-red-600">{row.reason || "Absent without approved leave"}</p>
                          )}
                        </div>
                      </div>
                    </li>
            ...
            <div className="border-t border-slate-100 px-6 py-4">
              <button type="button" onClick={() => setListModal(null)} className="w-full rounded-xl bg-slate-900 py-2.5 text-sm font-bold uppercase tracking-wide text-white hover:bg-slate-800">
                Close
              </button>
            </div>
          </div>

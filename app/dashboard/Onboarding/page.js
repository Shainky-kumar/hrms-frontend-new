...
//   joining_date: "",
//   tentative_joining_date: "",
//   permanent_address: { address_line: "", country: "", state: "", city: "", pincode: "" },
//   current_address: { address_line: "", country: "", state: "", city: "", pincode: "" },
//   experience: "",
//   source_of_hire: "",
//   title: "",
//   Skill_set: "",
...
//   if (Array.isArray(payload?.employment_types)) return payload.employment_types;
//   if (Array.isArray(payload?.results)) return payload.results;
//   if (Array.isArray(payload?.items)) return payload.items;
//   return [];
// };
...
//   if (!addr) return "—";
//   if (typeof addr === "string") return addr;
//   const parts = [addr.address_line, addr.city, addr.state, addr.country, addr.pincode].filter(Boolean);
//   return parts.length ? parts.join(", ") : "—";
// };
...
//   const [loadingConvertMasters, setLoadingConvertMasters] = useState(false);
...
//   const loadConvertMasters = useCallback(async () => {
//     setLoadingConvertMasters(true);
//     try {
//       const [locRes, empTypeRes, empRes] = await Promise.all([
//         api.get("/api/v1/get/location/master"),
//         api.get("/api/v1/get/employment-types"),
//         api.get("/api/v1/get/employees"),
//       ]);
//       setLocations(toArray(locRes?.data));
//       setEmploymentTypes(toArray(empTypeRes?.data));
//       setManagers(toArray(empRes?.data));
//     } catch (err) {
//       console.error("Failed to load convert masters", err);
//       setLocations([]);
//       setEmploymentTypes([]);
//       setManagers([]);
//     } finally {
//       setLoadingConvertMasters(false);
//     }
//   }, []);
...
//     setLoading(true);
//     try {
//       const res = await api.get("/api/v1/onboarding/employees", { params: { page, page_size: pageSize, search } });
//       const data = res?.data;
//       const items = toArray(data);
//       setList(items);
//       setTotal(data?.total || data?.total_count || items.length);
...
//   useEffect(() => { if (showForm) loadMasters(); }, [showForm, loadMasters]);
//   useEffect(() => { loadMasters(); }, []);
//   useEffect(() => { if (showConvert) loadConvertMasters(); }, [showConvert, loadConvertMasters]);
...
//   const openEdit = (item) => {
//     setEditId(item.on_boarding_id || item.id || item._id);
//     let eduText = typeof item.education_details === "string" ? item.education_details : item.education_details?.details || "";
//     let expText = typeof item.experience_details === "string" ? item.experience_details : item.experience_details?.details || "";
...
//       photo: item.photo || "",
//       joining_date: item.joining_date ? String(item.joining_date).slice(0, 10) : "",
//       tentative_joining_date: item.tentative_joining_date ? String(item.tentative_joining_date).slice(0, 10) : "",
//       permanent_address: item.permanent_address || initialForm.permanent_address,
//       current_address: item.current_address || initialForm.current_address,
//       experience: item.experience ?? "",
//       source_of_hire: item.source_of_hire || "",
//       title: item.title || "",
//       department: item.department || "",
//       Skill_set: Array.isArray(item.Skill_set) ? item.Skill_set.join(", ") : item.Skill_set || "",
//       highest_qualification: item.highest_qualification || "",
//       additional_info: item.additional_info || "",
//       current_salary: item.current_salary ?? "",
//       education_details: eduText,
...
//         title: formData.title || null,
//         department: formData.department || null,
//         Skill_set: formData.Skill_set ? formData.Skill_set.split(",").map((s) => s.trim()).filter(Boolean) : [],
//         highest_qualification: formData.highest_qualification || null,
//         additional_info: formData.additional_info || null,
//         current_salary: formData.current_salary ? Number(formData.current_salary) : null,
//         uploaded_documents: formData.uploaded_documents || [],
//         education_details: formData.education_details ? { details: formData.education_details } : null,
//         experience_details: formData.experience_details ? { details: formData.experience_details } : null,
//       };
...
//   const handleConvertChange = (field, value) => setConvertForm((prev) => ({ ...prev, [field]: value }));
...
//     try {
//       const payload = {
//         department_id: convertTarget.department || null,
//         designation_id: convertTarget.title || null,
//         location_id: convertForm.location_id,
//         employment_type_id: convertForm.employment_type_id || null,
//         reporting_manager: convertForm.reporting_manager || null,
//         password: convertForm.password || null,
//       };
//       const onBoardingId = convertTarget.on_boarding_id || convertTarget.id;
//       await api.post(`/api/v1/onboarding/${onBoardingId}/convert`, payload);
//       closeConvert();
//       await fetchData();
//     } catch (err) {
...
//     return (
//       <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${isConverted ? "bg-[#ecfdf5] text-[#059669]" : "bg-[#fffbeb] text-[#d97706]"}`}>
//         <span className={`h-1.5 w-1.5 rounded-full ${isConverted ? "bg-[#059669]" : "bg-[#d97706]"}`} />
...
//         <div>
//           <h1 className="text-[22px] font-semibold text-[#1a1a1a]">Candidates</h1>
//           <p className="mt-1 text-sm text-[#6b7280]">Manage pre-joining candidates and onboarding records</p>
//         </div>
//         <button onClick={openAdd} className="inline-flex items-center gap-2 rounded-md bg-[#E42527] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-[#c91f21]">
//           <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
//             <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
//           </svg>
//           Add Candidate
...
//             />
//           </div>
//           <div className="text-sm text-[#6b7280]">{total} candidate{total !== 1 ? "s" : ""}</div>
//         </div>

//         {/* SIMPLE TABLE — no more horizontal scroll of 24 columns */}
//         <div className="overflow-x-auto">
//           {loading ? (
//             <div className="flex items-center justify-center py-20">
//               <div className="flex items-center gap-3 text-sm text-[#6b7280]">
...
//               </div>
//               <p className="text-sm font-medium text-[#374151]">No candidates found</p>
//               <p className="mt-1 text-sm text-[#6b7280]">{search ? "Try a different search term" : "Add your first candidate to start onboarding"}</p>
//               {!search && (
//                 <button onClick={openAdd} className="mt-4 text-sm font-medium text-[#E42527] hover:underline">+ Add Candidate</button>
//               )}
//             </div>
//           ) : (
...
//               <thead>
//                 <tr className="border-b border-[#e5e7eb] bg-[#f9fafb]">
//                   <th className="px-4 py-3 font-medium text-[#6b7280]">Candidate</th>
//                   <th className="px-4 py-3 font-medium text-[#6b7280]">Email</th>
//                   <th className="px-4 py-3 font-medium text-[#6b7280]">Mobile</th>
...
//                   <th className="px-4 py-3 font-medium text-[#6b7280]">Joining Date</th>
...
//               <tbody className="divide-y divide-[#f3f4f6]">
//                 {list.map((item, index) => {
//                   const isConverted = item.conversion_status === "converted";
//                   return (
//                     <tr key={item.on_boarding_id || item.id || index} className="group transition hover:bg-[#fafafa]">
...
//                             {(item.last_name?.[0] || "").toUpperCase()}
//                           </div>
//                           <span className="font-medium text-[#1a1a1a]">{item.first_name} {item.last_name}</span>
//                         </div>
//                       </td>
//                       <td className="px-4 py-3 text-[#6b7280] whitespace-nowrap">{item.email || "—"}</td>
//                       <td className="px-4 py-3 text-[#6b7280] whitespace-nowrap">{item.mobile || "—"}</td>
...
//                       <td className="px-4 py-3 text-[#6b7280] whitespace-nowrap">{formatDate(item.joining_date)}</td>
//                       <td className="px-4 py-3 whitespace-nowrap">
//                         <StatusBadge status={item.conversion_status || "pending"} />
//                       </td>
//                       <td className="px-4 py-3 text-right whitespace-nowrap">
//                         <div className="flex items-center justify-end gap-1">
//                           <button
//                             onClick={() => setViewItem(item)}
//                             className="rounded-md px-2.5 py-1.5 text-xs font-medium text-[#6b7280] transition hover:bg-[#f3f4f6] hover:text-[#374151]"
//                           >
//                             View
//                           </button>
//                           <button
...
//             <div className="flex gap-2">
//               <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-md border border-[#d1d5db] px-3 py-1.5 text-sm font-medium text-[#374151] hover:bg-[#f9fafb] disabled:opacity-40">Previous</button>
//               <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-md border border-[#d1d5db] px-3 py-1.5 text-sm font-medium text-[#374151] hover:bg-[#f9fafb] disabled:opacity-40">Next</button>
//             </div>
//           </div>
//         )}
//       </div>
...
//             <div className="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
//               <div className="flex items-center gap-3">
//                 <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fef2f2] text-sm font-semibold text-[#E42527]">
//                   {(viewItem.first_name?.[0] || "C").toUpperCase()}
//                   {(viewItem.last_name?.[0] || "").toUpperCase()}
//                 </div>
//                 <div>
//                   <h2 className="text-base font-semibold text-[#1a1a1a]">{viewItem.first_name} {viewItem.last_name}</h2>
//                   <p className="mt-0.5 text-sm text-[#6b7280]">{viewItem.email}</p>
//                 </div>
//               </div>
//               <button onClick={() => setViewItem(null)} className="rounded-md p-1.5 text-[#9ca3af] hover:bg-[#f3f4f6]">
//                 <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
//                   <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
//                 </svg>
//               </button>
...
//                 <InfoRow label="Personal Email" value={viewItem.email} />
//                 <InfoRow label="Personal Mobile" value={viewItem.mobile} />
//                 <InfoRow label="Official Email" value={viewItem.Official_Email} />
//                 <InfoRow label="Official Mobile" value={viewItem.Official_Mobile} />
//                 <InfoRow label="Department" value={getDeptName(viewItem.department)} />
//                 <InfoRow label="Designation" value={getDesigName(viewItem.title)} />
//                 <InfoRow label="Joining Date" value={formatDate(viewItem.joining_date)} />
//                 <InfoRow label="Tentative Joining" value={formatDate(viewItem.tentative_joining_date)} />
//                 <InfoRow label="Experience" value={viewItem.experience != null ? `${viewItem.experience} yrs` : "—"} />
//                 <InfoRow label="Current Salary" value={viewItem.current_salary != null ? `₹ ${viewItem.current_salary}` : "—"} />
//                 <InfoRow label="Source of Hire" value={viewItem.source_of_hire} />
//                 <InfoRow label="Highest Qualification" value={viewItem.highest_qualification} />
//                 <InfoRow label="Aadhaar" value={viewItem.aadhaar_number} />
//                 <InfoRow label="PAN" value={viewItem.pan_number} />
//                 <InfoRow label="Status" value={viewItem.conversion_status === "converted" ? "Converted" : "Pending"} />
//                 <InfoRow label="Skills" value={Array.isArray(viewItem.Skill_set) ? viewItem.Skill_set.join(", ") : viewItem.Skill_set} full />
//                 <InfoRow label="Permanent Address" value={formatAddr(viewItem.permanent_address)} full />
//                 <InfoRow label="Current Address" value={formatAddr(viewItem.current_address)} full />
//                 <InfoRow label="Education Details" value={getDetails(viewItem.education_details)} full />
//                 <InfoRow label="Experience Details" value={getDetails(viewItem.experience_details)} full />
//                 <InfoRow label="Additional Info" value={viewItem.additional_info} full />
//               </div>
//             </div>

//             <div className="flex items-center justify-end gap-3 border-t border-[#e5e7eb] px-6 py-4">
//               <button onClick={() => setViewItem(null)} className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">Close</button>
//               <button
...
//             <div className="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
//               <div>
//                 <h2 className="text-lg font-semibold text-[#1a1a1a]">{editId ? "Edit Candidate" : "Add Candidate"}</h2>
//                 <p className="mt-0.5 text-sm text-[#6b7280]">Fill candidate details for onboarding</p>
//               </div>
//               <button onClick={closeModal} className="rounded-md p-1.5 text-[#9ca3af] hover:bg-[#f3f4f6]">
//                 <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
//                   <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
...
//             <div className="flex border-b border-[#e5e7eb] px-6">
//               {tabs.map((tab) => (
//                 <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`relative px-4 py-3 text-sm font-medium transition ${activeTab === tab.id ? "text-[#E42527]" : "text-[#6b7280] hover:text-[#374151]"}`}>
//                   {tab.label}
//                   {activeTab === tab.id && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#E42527]" />}
//                 </button>
//               ))}
//             </div>
...
//                   <div className="grid gap-4 sm:grid-cols-2">
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">First Name <span className="text-[#E42527]">*</span></label>
//                       <input required value={formData.first_name} onChange={(e) => handleChange("first_name", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Last Name <span className="text-[#E42527]">*</span></label>
//                       <input required value={formData.last_name} onChange={(e) => handleChange("last_name", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Personal Email <span className="text-[#E42527]">*</span></label>
//                       <input required type="email" value={formData.email} onChange={(e) => handleChange("email", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Personal Mobile <span className="text-[#E42527]">*</span></label>
//                       <input required value={formData.mobile} onChange={(e) => handleChange("mobile", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Official Email <span className="text-[#E42527]">*</span></label>
//                       <input required type="email" value={formData.Official_Email} onChange={(e) => handleChange("Official_Email", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Official Mobile <span className="text-[#E42527]">*</span></label>
//                       <input required value={formData.Official_Mobile} onChange={(e) => handleChange("Official_Mobile", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Aadhaar Number</label>
//                       <input value={formData.aadhaar_number} onChange={(e) => handleChange("aadhaar_number", e.target.value.replace(/\D/g, "").slice(0, 12))} inputMode="numeric" maxLength={12} pattern="[0-9]{12}" placeholder="12 digits" className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">PAN Number</label>
//                       <input value={formData.pan_number} onChange={(e) => handleChange("pan_number", e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 10))} maxLength={10} pattern="[A-Z]{5}[0-9]{4}[A-Z]" placeholder="ABCDE1234F" className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm uppercase focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                   </div>
//                 )}
...
//                       <select value={formData.title} onChange={(e) => handleChange("title", e.target.value)} disabled={loadingMasters} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50">
//                         <option value="">{loadingMasters ? "Loading..." : "Select Designation"}</option>
//                         {designations.map((d) => (<option key={d.designation_id || d.id} value={d.designation_id || d.id}>{d.job_title || d.designation_name || d.name}</option>))}
//                       </select>
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Joining Date</label>
...
//                       <h4 className="mb-3 text-sm font-semibold text-[#374151]">Permanent Address</h4>
//                       <div className="grid gap-4 sm:grid-cols-2">
//                         <div className="sm:col-span-2"><input placeholder="Address Line" value={formData.permanent_address.address_line} onChange={(e) => handleAddressChange("permanent_address", "address_line", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" /></div>
//                         <input placeholder="Country" value={formData.permanent_address.country} onChange={(e) => handleAddressChange("permanent_address", "country", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="State" value={formData.permanent_address.state} onChange={(e) => handleAddressChange("permanent_address", "state", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="City" value={formData.permanent_address.city} onChange={(e) => handleAddressChange("permanent_address", "city", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="Pincode" value={formData.permanent_address.pincode} onChange={(e) => handleAddressChange("permanent_address", "pincode", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
...
//                       <h4 className="mb-3 text-sm font-semibold text-[#374151]">Current Address</h4>
//                       <div className="grid gap-4 sm:grid-cols-2">
//                         <div className="sm:col-span-2"><input placeholder="Address Line" value={formData.current_address.address_line} onChange={(e) => handleAddressChange("current_address", "address_line", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" /></div>
//                         <input placeholder="Country" value={formData.current_address.country} onChange={(e) => handleAddressChange("current_address", "country", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="State" value={formData.current_address.state} onChange={(e) => handleAddressChange("current_address", "state", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="City" value={formData.current_address.city} onChange={(e) => handleAddressChange("current_address", "city", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                         <input placeholder="Pincode" value={formData.current_address.pincode} onChange={(e) => handleAddressChange("current_address", "pincode", e.target.value)} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
...
//                   <div className="grid gap-4 sm:grid-cols-2">
//                     <div className="sm:col-span-2">
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Skill Set <span className="font-normal text-[#9ca3af]">(comma separated)</span></label>
//                       <input value={formData.Skill_set} onChange={(e) => handleChange("Skill_set", e.target.value)} placeholder="React, Node.js, Python" className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                     </div>
//                     <div>
//                       <label className="mb-1.5 block text-sm font-medium text-[#374151]">Highest Qualification</label>
...
//                 <div className="flex gap-2">
//                   {activeTab !== "basic" && (
//                     <button type="button" onClick={() => { const idx = tabs.findIndex((t) => t.id === activeTab); setActiveTab(tabs[idx - 1].id); }} className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">Previous</button>
//                   )}
//                   {activeTab !== "other" && (
//                     <button type="button" onClick={() => { const idx = tabs.findIndex((t) => t.id === activeTab); setActiveTab(tabs[idx + 1].id); }} className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">Next</button>
//                   )}
//                 </div>
//                 <div className="flex gap-3">
//                   <button type="button" onClick={closeModal} className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">Cancel</button>
//                   <button type="submit" disabled={saving} className="rounded-md bg-[#E42527] px-5 py-2 text-sm font-medium text-white hover:bg-[#c91f21] disabled:opacity-60">{saving ? "Saving..." : editId ? "Update Candidate" : "Add Candidate"}</button>
//                 </div>
//               </div>
//             </form>
//           </div>
...
//             <div className="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
//               <div>
//                 <h2 className="text-base font-semibold text-[#1a1a1a]">Convert to Employee</h2>
//                 <p className="mt-0.5 text-sm text-[#6b7280]">{convertTarget.first_name} {convertTarget.last_name}</p>
//               </div>
//               <button onClick={closeConvert} className="rounded-md p-1.5 text-[#9ca3af] hover:bg-[#f3f4f6]">
//                 <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
//                   <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
//                 </svg>
//               </button>
...
//                     <span className="font-medium text-[#1a1a1a]">{getDeptName(convertTarget.department)}</span>
//                     <span className="text-[#6b7280]">Designation</span>
//                     <span className="font-medium text-[#1a1a1a]">{getDesigName(convertTarget.title)}</span>
//                   </div>
//                 </div>

//                 <div>
//                   <label className="mb-1.5 block text-sm font-medium text-[#374151]">Location <span className="text-[#E42527]">*</span></label>
//                   <select required value={convertForm.location_id} onChange={(e) => handleConvertChange("location_id", e.target.value)} disabled={loadingConvertMasters} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50">
//                     <option value="">{loadingConvertMasters ? "Loading..." : "Select Location"}</option>
...
//                   <select value={convertForm.employment_type_id} onChange={(e) => handleConvertChange("employment_type_id", e.target.value)} disabled={loadingConvertMasters} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50">
//                     <option value="">{loadingConvertMasters ? "Loading..." : "Select Employment Type"}</option>
//                     {employmentTypes.map((et) => (<option key={et.employment_type_id || et.id} value={et.employment_type_id || et.id}>{et.employment_type_name || et.name}</option>))}
//                   </select>
//                 </div>

//                 <div>
//                   <label className="mb-1.5 block text-sm font-medium text-[#374151]">Reporting Manager</label>
//                   <select value={convertForm.reporting_manager} onChange={(e) => handleConvertChange("reporting_manager", e.target.value)} disabled={loadingConvertMasters} className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50">
//                     <option value="">{loadingConvertMasters ? "Loading..." : "Select Manager (optional)"}</option>
//                     {managers.map((m) => (<option key={m.employee_id || m.id} value={m.employee_id || m.id}>{m.employee_name || m.name || m.employee_id}</option>))}
//                   </select>
//                 </div>

//                 <div>
//                   <label className="mb-1.5 block text-sm font-medium text-[#374151]">Login Password <span className="font-normal text-[#9ca3af]">(leave blank if user exists)</span></label>
//                   <input type="password" value={convertForm.password} onChange={(e) => handleConvertChange("password", e.target.value)} placeholder="Set a login password" className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//                 </div>

//                 {convertError && <div className="rounded-md bg-[#fef2f2] px-3 py-2.5 text-sm text-[#b91c1c]">{convertError}</div>}
//               </div>

//               <div className="flex items-center justify-end gap-3 border-t border-[#e5e7eb] px-6 py-4">
//                 <button type="button" onClick={closeConvert} className="rounded-md border border-[#d1d5db] px-4 py-2 text-sm font-medium text-[#374151] hover:bg-[#f9fafb]">Cancel</button>
//                 <button type="submit" disabled={converting} className="rounded-md bg-[#E42527] px-5 py-2 text-sm font-medium text-white hover:bg-[#c91f21] disabled:opacity-60">{converting ? "Converting..." : "Confirm & Convert"}</button>
//               </div>
//             </form>
//           </div>
//         </div>
...
// // ---- Small helper component for View modal rows ----
// function InfoRow({ label, value, full = false }) {
//   return (
//     <div className={full ? "col-span-2" : ""}>
//       <p className="text-xs text-[#9ca3af]">{label}</p>
//       <p className="mt-0.5 text-[#1a1a1a]">{value || "—"}</p>
//     </div>
//   );
// }
...
const toArray = (payload) => {
  ...
  if (Array.isArray(payload?.designations)) return payload.designations;
  if (Array.isArray(payload?.locations)) return payload.locations;
  if (Array.isArray(payload?.location_master)) return payload.location_master;
  if (Array.isArray(payload?.employment_types)) return payload.employment_types;
  if (Array.isArray(payload?.employment_type)) return payload.employment_type;
  if (Array.isArray(payload?.results)) return payload.results;
  if (Array.isArray(payload?.items)) return payload.items;
  return [];
};
...
export default function CandidatesPage() {
  ...
  // ================= FIXED: sahi API paths ke saath, Promise.allSettled use kiya =================
  const loadConvertMasters = useCallback(async () => {
    setLoadingConvertMasters(true);
    try {
      const results = await Promise.allSettled([
        api.get("/api/v1/get/location/master"),
        api.get("/api/v1/get/employment/type"),
        api.get("/api/v1/get/employees"),
      ]);

      const [locRes, empTypeRes, empRes] = results;

      // Debug logs - console me response dekhne ke liye
      console.log(
        "Location result:",
        locRes.status === "fulfilled" ? locRes.value?.data : locRes.reason
      );
      console.log(
        "Employment type result:",
        empTypeRes.status === "fulfilled" ? empTypeRes.value?.data : empTypeRes.reason
      );
      console.log(
        "Employees result:",
        empRes.status === "fulfilled" ? empRes.value?.data : empRes.reason
      );

      setLocations(locRes.status === "fulfilled" ? toArray(locRes.value?.data) : []);
      setEmploymentTypes(empTypeRes.status === "fulfilled" ? toArray(empTypeRes.value?.data) : []);
      setManagers(empRes.status === "fulfilled" ? toArray(empRes.value?.data) : []);

      if (locRes.status === "rejected") console.error("Location API failed:", locRes.reason);
      if (empTypeRes.status === "rejected") console.error("Employment type API failed:", empTypeRes.reason);
      if (empRes.status === "rejected") console.error("Employees API failed:", empRes.reason);
    } catch (err) {
      console.error("Unexpected error loading convert masters", err);
    } finally {
      setLoadingConvertMasters(false);
    }
  }, []);
              ...
              <div className="max-h-[65vh] overflow-y-auto px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleConvertChange("location_id", e.target.value)}
                    disabled={loadingConvertMasters}
                  ...
                  >
                    <option value="">
                      {loadingConvertMasters ? "Loading..." : locations.length === 0 ? "No locations found" : "Select Location"}
                    </option>
                    {locations.map((l, idx) => (
                      <option key={l.location_id || l.id || idx} value={l.location_id || l.id}>
                        {l.location_name || l.name || l.city || l.location || `Location ${idx + 1}`}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#374151]">Employment Type</label>
                  <select
                    value={convertForm.employment_type_id}
                    onChange={(e) => handleConvertChange("employment_type_id", e.target.value)}
                    disabled={loadingConvertMasters}
                    className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50"
                  >
                    <option value="">
                      {loadingConvertMasters ? "Loading..." : employmentTypes.length === 0 ? "No employment types found" : "Select Employment Type"}
                    </option>
                    {employmentTypes.map((et, idx) => (
                      <option key={et.employment_type_id || et.id || idx} value={et.employment_type_id || et.id}>
                        {et.employment_type_name || et.name || et.type || `Type ${idx + 1}`}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#374151]">Reporting Manager</label>
                  <select
                    value={convertForm.reporting_manager}
                    onChange={(e) => handleConvertChange("reporting_manager", e.target.value)}
                    disabled={loadingConvertMasters}
                    className="w-full rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527] disabled:bg-gray-50"
                  >
                    <option value="">
                      {loadingConvertMasters ? "Loading..." : "Select Manager (optional)"}
                    </option>
                    {managers.map((m, idx) => (
                      <option key={m.employee_id || m.id || idx} value={m.employee_id || m.id}>
                        {m.employee_name || m.name || m.employee_id}
                      </option>
                    ))}
                  </select>
                </div>

...
const extractList = (res) => {
  ...
  if (Array.isArray(data.regularizations))
    return {
      ...
      total: Number(data.total) || data.regularizations.length,
    };
  if (Array.isArray(data.data))
    return { items: data.data, total: Number(data.total) || data.data.length };
  if (Array.isArray(data.items))
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        disabled={loading}
        className="flex w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-left text-sm focus:border-red-500 focus:outline-none disabled:cursor-not-allowed disabled:bg-slate-50"
      >
        <span className={selected ? "text-slate-700" : "text-slate-400"}>
          {loading
            ? "Loading employees..."
      ...
      {open && (
        <div className="absolute z-40 mt-1 w-full rounded-lg border border-slate-200 bg-white shadow-lg">
          ...
          <div className="max-h-64 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <div className="px-3 py-6 text-center text-xs text-slate-400">
                {query.trim() ? "No matches" : "No employees found"}
              </div>
            ) : (
              filtered.map((e) => (
                <button
          ...
          <div className="border-t border-slate-100 bg-slate-50 px-3 py-1.5 text-[10px] text-slate-500">
            {filtered.length} of {options.length} employees
          </div>
        </div>
      )}
    </div>
...
/* ══════════════════════════════════════════════════════════
   CREATE MODAL — role-based, uses resolved employee ID
   ══════════════════════════════════════════════════════════ */
...
function CreateModal({
  ...
  useEffect(() => {
    ...
    const prefill = isAdmin ? "" : (myEmployeeId || "");
  ...
  const submit = async (e) => {
    ...
    if (!form.attendance_date) return setError("Please select a date");
    if (!form.reason.trim()) return setError("Please provide a reason");

    const punchInIso = buildIso(form.attendance_date, form.requested_punch_in);
    const punchOutIso = buildIso(
      form.attendance_date,
      form.requested_punch_out
    );
  ...
  const isSelfLocked = !isAdmin && myEmployeeId;
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              {isAdmin
                ? "Create a correction request for any employee"
                : "Request correction for your own attendance"}
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={submit} className="space-y-4 p-5">
          ...
          <div>
            ...
            {isAdmin ? (
              ...
              /* USER → locked self card */
              <div className="flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2.5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-blue-700 text-[11px] font-bold text-white">
              ...
              /* Could not resolve employee ID */
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                <p className="text-xs font-medium text-amber-800">
                  ⚠️ Your employee profile is not linked
                </p>
                <p className="mt-0.5 text-[11px] text-amber-700">
                  Please contact HR to link your account with an employee profile.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
            ...
            <button
              type="submit"
              disabled={saving || (!isAdmin && !myEmployeeId)}
              className="rounded-lg bg-red-600 px-5 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-60"
            >
              {saving ? "Submitting..." : "Submit Request"}
            </button>
...
export default function RegularizationPage() {
  ...
  const roleRaw = user?.role?.value || user?.role || "";
  ...
  /* ⭐ Resolved own employee ID — from API if not in store */
  const [myEmployeeId, setMyEmployeeId] = useState("");
  const [resolvingSelf, setResolvingSelf] = useState(true);
  ...
  /* ══════════════════════════════════════════════════════════
     RESOLVE OWN EMPLOYEE ID
     1. Try user.employee_id from store
     2. Fallback: match user.user_id against employees list
     ══════════════════════════════════════════════════════════ */
  useEffect(() => {
    const fromStore =
      user?.employee_id ||
    ...
    if (fromStore) {
      setMyEmployeeId(String(fromStore));
    ...
    if (me?.id) {
      setMyEmployeeId(String(me.id));

      // Optionally save back to store for next time
      try {
        const store = useAuthStore.getState();
        if (store?.setUser && user) {
          store.setUser({ ...user, employee_id: me.id });
        }
      } catch {}
    } else {
      setMyEmployeeId("");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setEmployees(list);
        if (list.length === 0) setEmpError("No employees found.");
      } catch (err) {
        if (cancelled) return;
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      if (statusFilter !== "all") params.status = statusFilter;
      if (isAdmin && employeeId) params.employee_id = employeeId;
      if (!isAdmin && myEmployeeId) params.employee_id = myEmployeeId;
  ...
  useEffect(() => {
    // Don't fetch until self-resolution completes for non-admin
    if (!isAdmin && resolvingSelf) return;
    ...
    const t = setTimeout(fetchList, DEBOUNCE_MS);
    return () => clearTimeout(t);
  }, [fetchList, isAdmin, resolvingSelf]);

  const getEmpName = useCallback(
    (id) => {
      if (!id) return "—";
      const e = employees.find((x) => x.id === String(id));
      return e ? e.name : id;
    },
    [employees]
  );
  ...
  const selectedList = Array.from(selectedIds);
  const allSelected =
    pendingIds.length > 0 && selectedIds.size === pendingIds.length;
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => setCreating(true)}
              disabled={!isAdmin && resolvingSelf}
        ...
        {empError && (
          <div className="mb-4 flex items-start justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <div>
              ...
              <p className="mt-0.5 text-xs">{empError}</p>
            </div>
            <button
              type="button"
              onClick={() => window.location.reload()}
              className="shrink-0 rounded border border-amber-300 bg-white px-3 py-1 text-xs font-medium hover:bg-amber-100"
            >
              Retry
            </button>
          </div>
            ...
            {loading && list.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
                ...
                <button
                  ...
                  onClick={() => setCreating(true)}
                  disabled={!isAdmin && resolvingSelf}
                  className="mt-1 rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-60"
                >
                  + New Request
                </button>
              </div>
                        ...
                        <td className="px-5 py-3.5">
                          <div className="text-xs text-slate-600">
                            <div>
                              Status:{" "}
                              <span className="font-medium text-slate-800">
                                {requestedStatusLabel(row.requested_status)}
                              </span>
                            </div>
                            {(row.requested_punch_in ||
                              row.requested_punch_out) && (
                              <div className="mt-0.5 text-[11px] text-slate-500">
                                In: {formatTime(row.requested_punch_in, tz)} ·
                                Out: {formatTime(row.requested_punch_out, tz)}
                              </div>
                            )}
                          </div>
                        </td>

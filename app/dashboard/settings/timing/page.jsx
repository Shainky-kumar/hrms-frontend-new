...
const getTimingId = (item) =>
  item?.company_timing_id || item?.timing_id || item?.id || item?._id;
...
const isNightShift = (start, end) => {
  ...
  const s = formatInputTime(start).split(":").map(Number);
  const e = formatInputTime(end).split(":").map(Number);
...
const IconBtn = ({ icon, title, onClick, danger, disabled }) => (
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`rounded-md border border-transparent p-1.5 transition disabled:cursor-not-allowed disabled:opacity-50 ${
      danger
...
const ModalShell = ({ title, subtitle, onClose, children, footer }) => (
  ...
  >
    <div
      className="mb-10 w-full max-w-md overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl"
      role="dialog"
    >
...
const inputCls =
  "w-full rounded-md border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-800 outline-none transition focus:border-[#E42527] focus:ring-2 focus:ring-red-100";
...
export default function TimingPage() {
  ...
  const handleDelete = async (id) => {
    ...
    if (!window.confirm("Kya aap ye timing delete karna chahte ho?")) return;
  ...
  const totalHours = list.reduce((sum, item) => {
    ...
    const s = formatInputTime(item.start_time).split(":").map(Number);
    const e = formatInputTime(item.end_time).split(":").map(Number);
    let startTotal = s[0] * 60 + s[1];
    let endTotal = e[0] * 60 + e[1];
    if (endTotal <= startTotal) endTotal += 24 * 60;
    return sum + (endTotal - startTotal) / 60;
  }, 0);

  const nightCount = list.filter((item) =>
    isNightShift(item.start_time, item.end_time)
  ).length;
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] p-4 sm:p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-4">
        ...
        <div className="rounded-lg border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-5">
          <div className="mb-3 flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <Icons.Chevron />
            <span className="font-medium text-slate-700">Company Timings</span>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-[13px] text-slate-500">
                Manage office working hours for shift scheduling
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard label="Total Timings" value={list.length} icon="🕒" tone="blue" />
          <StatCard label="Day Timings" value={list.length - nightCount} icon="☀️" tone="amber" />
          <StatCard label="Night Timings" value={nightCount} icon="🌙" tone="violet" />
          <StatCard
            label="Total Hours"
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-semibold text-slate-800">Saved Timings</h2>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm font-semibold text-slate-700">
                {search ? "No timings match your search" : "No timings yet"}
              </p>
              <p className="mt-1 text-xs text-slate-500">
                {search
                  ? "Try a different keyword."
                  : "Create your first timing to get started."}
              </p>
              {!search && (
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">
              {filteredList.map((item, index) => {
                ...
                const night = isNightShift(item.start_time, item.end_time);

                return (
                  <div
                    key={timingId || index}
                    className="group relative overflow-hidden rounded-lg border border-slate-200 bg-white p-5 transition hover:border-[#E42527]/30 hover:shadow-md"
                  >
                    {/* Decorative corner */}
                    ...
                    {/* Top row */}
                    <div className="relative flex items-start justify-between">
                      <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-red-50 text-[#E42527]">
                        <Icons.Clock />
                      </div>
                      <Badge tone={night ? "violet" : "green"} dot>
                        {night ? "Night" : "Day"}
                      </Badge>
                    </div>
      ...
      {showForm && (
        <ModalShell
          title={editId ? "Edit Timing" : "Add Timing"}
          subtitle="Office ka working hours select karo"
          onClose={closeForm}
          footer={
            <>
              ...
              <Btn type="submit" form="timing-form" disabled={saving}>
                {saving ? "Saving..." : editId ? "Update Timing" : "Add Timing"}
              </Btn>
            </>
          }
        >
          <form id="timing-form" onSubmit={handleSubmit} className="space-y-4">
            {error && (
            ...
            <div className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <div className="flex items-center justify-center gap-3 text-sm font-semibold text-slate-700">
                <span>
                  {formData.start_time ? formatTime(formData.start_time) : "Start"}
                </span>
                <span className="text-slate-300">
                  <Icons.ArrowRight />
                </span>
                <span>
                  {formData.end_time ? formatTime(formData.end_time) : "End"}
                </span>
              </div>
              {formData.start_time && formData.end_time && (
                <p className="mt-1.5 text-center text-[11px] text-slate-500">
                  Duration: {calculateDuration(formData.start_time, formData.end_time)}
                  {isNightShift(formData.start_time, formData.end_time) && " · Night Shift"}
                </p>
              )}
            </div>

            <p className="text-[11px] text-slate-500">
              Agar end time start time se chhota hai to ye night shift maana jayega.
            </p>
          </form>
        </ModalShell>
      )}

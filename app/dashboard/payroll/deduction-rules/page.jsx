...
// ⭐ NEW: Base component options
const BASE_COMPONENTS = [
  { value: "GROSS", label: "Gross (Full Earnings)", hint: "Deduct on total earnings" },
  { value: "BASIC", label: "Basic Salary", hint: "Deduct on Basic only (recommended for PF)" },
];
...
function getRulesList(response) {
  const body = response?.data ?? {};
  const list = body?.data ?? body?.rules ?? body?.deduction_rules ?? body?.items ?? body?.results ?? [];
  return Array.isArray(list) ? list : [];
}
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const payload = {
        ...
        wage_ceiling: form.wage_ceiling !== "" ? Number(form.wage_ceiling) : null,
        wage_floor: form.wage_floor !== "" ? Number(form.wage_floor) : null,
        // ⭐ NEW: base component code
        base_component_code: form.base_component_code || "GROSS",
        slab_config: isSlab
          ? slabRows
              .filter((r) => r.min_wage !== "" || r.max_wage !== "" || r.value !== "")
              .map((r) => ({
            ...
            {listLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/80">
                      <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Rule</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Type</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Base</th>
                      <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">Value</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Effective</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Flags</th>
                      <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {rules.map((rule, idx) => {
                      const name = rule.deduction_name || rule.name || rule.rule_name || "Rule";
                      const code = rule.code || "—";
                      const calcType = rule.calculation_type || "—";
                      const baseCode = rule.base_component_code || "GROSS";

                      let valueDisplay = "—";
                      if (calcType === "slab") {
                        const count = Array.isArray(rule.slab_config) ? rule.slab_config.length : 0;
                        valueDisplay = count > 0 ? `${count} slabs` : "—";
                      } else if (rule.deducted_value != null) {
                        valueDisplay =
                          calcType === "percentage"
                      ...
                      return (
                        <tr key={rule.deduction_rule_id || rule.id || idx} className="hover:bg-slate-50/70">
                          ...
                          <td className="px-5 py-3.5 text-slate-600">
                            <div>{formatDate(rule.effective_from)}</div>
                            <div className="text-xs text-slate-400">
                              to {rule.effective_to ? formatDate(rule.effective_to) : "Ongoing"}
                            </div>
                          </td>
                          <td className="px-5 py-3.5">
                            <div className="flex flex-wrap gap-1">
                              {rule.is_statutory && (
                                <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">Statutory</span>
                              )}
                              {rule.is_employer_contribution && (
                                <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-medium text-amber-700">Employer</span>
                              )}
                              {rule.is_active !== false && (
          ...
          <div className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {/* ⭐ NEW: Base Component dropdown */}
                {form.calculation_type !== "slab" && (
                  <div className="sm:col-span-2">
                    <label className="mb-1.5 block text-sm font-medium text-slate-700">
                      Base Component *{" "}
                      <span className="text-xs font-normal text-slate-400">
                        (which amount should this be calculated on?)
                      </span>
                    </label>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {BASE_COMPONENTS.map((opt) => (
                        <label
                          key={opt.value}
                          className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${
                            form.base_component_code === opt.value
                              ? "border-[#E42527] bg-red-50/50"
                        ...
                        >
                          <input
                            ...
                            value={opt.value}
                            checked={form.base_component_code === opt.value}
                            onChange={(e) =>
                              updateField("base_component_code", e.target.value)
                            }
                            className="mt-0.5"
                          />
                          <div>
                    ...
                    {form.base_component_code === "BASIC" && (
                      <p className="mt-2 text-xs text-purple-700">
                        ⭐ Recommended for <b>PF</b> (statutory requires Basic)
                      </p>
                    )}
                    {form.base_component_code === "GROSS" && (
                      <p className="mt-2 text-xs text-slate-500">
                        Default for <b>ESI, PT, TDS</b> and general deductions
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-[#E42527]"
                  />
                  <p className="mt-1 text-xs text-slate-400">Leave blank if ongoing</p>
                </div>
              </div>
          ...
          <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="grid max-h-[60vh] gap-3 overflow-y-auto p-5 sm:grid-cols-2">
              {[
                ["Code", selectedRule.code],
                ["Calculation Type", prettify(selectedRule.calculation_type)],
                ["Base Component", selectedRule.base_component_code === "BASIC" ? "Basic" : "Gross"],
                ["Value", selectedRule.deducted_value],
                ["Wage Floor", selectedRule.wage_floor],
                ["Wage Ceiling", selectedRule.wage_ceiling],
                ["Effective From", formatDate(selectedRule.effective_from)],
                ["Effective To", selectedRule.effective_to ? formatDate(selectedRule.effective_to) : "Ongoing"],
                ["Statutory", selectedRule.is_statutory ? "Yes" : "No"],
                ["Employer Contribution", selectedRule.is_employer_contribution ? "Yes" : "No"],
                ["Active", selectedRule.is_active !== false ? "Yes" : "No"],
              ].map(([label, value]) => (
                <div key={label} className="rounded-xl bg-slate-50 px-3 py-2.5">
                  <p className="text-xs text-slate-400">{label}</p>
                      ...
                      {selectedRule.slab_config.map((s, i) => (
                        ...
                        >
                          <span className="text-slate-600">
                            ₹{s.min_wage ?? s.from ?? 0}
                            {" – "}
                            {s.max_wage != null || s.to != null ? `₹${s.max_wage ?? s.to}` : "∞"}
                          </span>
                          <span className="font-semibold text-slate-800">
                            {s.value ?? s.rate ?? 0}%
                          </span>
